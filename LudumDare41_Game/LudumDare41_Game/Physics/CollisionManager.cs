...
namespace LudumDare41_Game.Physics {
    class CollisionManager {
        ...
        public static void DrawDebugRectangleCollider (SpriteBatch sb, RectangleCollider collider, Color c) {

            Texture2D pixel = new Texture2D(sb.GraphicsDevice, 40, 40);
            pixel.SetData(SetDebugDrawColorData(pixel.Width, pixel.Height, Color.White));

            sb.Begin();
            sb.Draw(pixel, new Rectangle((int)collider.Position.X, (int)collider.Position.Y, collider.Width, collider.Height), null, Color.White, collider.Rotation, new Vector2(pixel.Width / 2, pixel.Height / 2), SpriteEffects.None, 1);
            sb.End();
        }

        private static Color[] SetDebugDrawColorData (int _width, int _height, Color c) {
            Color[] temp = new Color[_width * _height];
